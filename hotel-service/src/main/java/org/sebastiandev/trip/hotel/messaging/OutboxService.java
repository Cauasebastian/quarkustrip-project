...
import org.sebastiandev.trip.contracts.event.*; import org.sebastiandev.trip.hotel.repository.OutboxRepository;
@ApplicationScoped public class OutboxService {
 ...
 public Uni<EventEnvelope> enqueue(String topic, UUID aggregateId, UUID cause, Object payload) {
  ...
  event.id=envelope.eventId(); event.topic=topic; event.aggregateId=aggregateId; event.payload=EventCodec.encode(mapper,envelope); event.createdAt=OffsetDateTime.now(ZoneOffset.UTC);
